...
CTFd._internal.challenge.setup = function() {
    ...
    window.instancier.oracle_button.on("click", function(_event) {
        // Post an attempt to the challenge
        CTFd._internal.challenge.submit().then(function(response) {
            if (response.status === 429) {
                CTFd.ui.ezq.push("error", "Error", response.data.message);
            } else if (response.status === 403) {
                CTFd.ui.ezq.push("error", "Error", response.data.message);
            } else {
                CTFd.ui.ezq.push("success", "Success", "Challenge solved successfully");
            }
        });
    });
